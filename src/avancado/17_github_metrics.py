...
from scripts.collectors.github_api        import collect_all
from scripts.parsing.github.processor     import process_all
...
def build_aba_dashboards(df_dash):
    ...
    fig = make_subplots(rows=1, cols=2,
        subplot_titles=("Commits por tipo", "Commits por escopo"),
...
def build_aba_cisco(df_cisco):
    ...
    fig2 = go.Figure(go.Bar(
        x=cmo["month"], y=cmo["count"],
        marker_color=COLOR_CISCO,
...
def build_aba_correlacao(df_all):
    ...
    weeks      = all_weeks
    dash_vals  = [dash_map.get(w, 0)  for w in weeks]
    cisco_vals = [cisco_map.get(w, 0) for w in weeks]
...
<body>
  ...
  <p class="subtitle">Evolução contínua — DASHBOARDS + CISCO em paralelo</p>
  <div class="tabs">
    <button class="tab-btn active" onclick="showTab('geral')">Visão Geral</button>
    <button class="tab-btn" onclick="showTab('dashboards')">DASHBOARDS</button>
    <button class="tab-btn" onclick="showTab('cisco')">CISCO</button>
    <button class="tab-btn" onclick="showTab('correlacao')">Correlação</button>
  </div>
...
def main():
    ...
    if all(os.path.exists(p) for p in [csv_all, csv_dash, csv_cis, json_inf]):
        print("[17] Dados já processados — carregando CSVs...")
